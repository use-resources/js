  ...
  static __script = (params) => {
    ...
    $elements.video.addEventListener("error", () => {
      ...
      if ($elements.video.getAttribute("src") != "") {
        styleElement($elements.spanLoad, {
          display: "none",
        });

        styleElement($elements.spanError, {
          display: "",
        });
      }
    });
    ...
    $elements.video.addEventListener("loadedmetadata", () => {
      ...
      $elements.video.play().then(console.log).catch(console.log);
    });

    $elements.video.addEventListener("durationchange", () => {
      const convert = convertSecondsToTime($elements.video.duration);
    ...
    $elements.video.addEventListener("timeupdate", () => {
      ...
      styleElement($elements.durationBar, {
        width:
          getPercentage($elements.video.currentTime, $elements.video.duration) +
          "%",
      });
    ...
    $elements.divTap.addEventListener("click", () => {
    ...
    $elements.divOptionsPopupSkipTime.addEventListener("click", (e) => {
      if (e.target == e.currentTarget) {
        e.target.hidePopover();
      }
    });

    $elements.divOptionsPopupBackgroundAmbient.addEventListener(
      "click",
      (e) => {
        if (e.target == e.currentTarget) {
          e.target.hidePopover();
    ...
    $elements.divOptionsPopupSpeed.addEventListener("click", (e) => {
      if (e.target == e.currentTarget) {
        e.target.hidePopover();
      }
    });
    ...
    $elements.divOptionsButtons.addEventListener("click", (e) => {
      ...
      if (button) {
        ...
        if (action == "download") {
          ...
          downloadFile($elements.video.getAttribute("src"), "video.mp4");
        }

        if (action == "resize") {
          return styleElement($elements.video, {
            objectFit: toggleOptions(
